...
>   - сделал базовый `InMemoryBaseRepositoryImpl` 
>   - наследую от него `InMemoryUserRepositoryImpl`
>   - использую его в `InMemoryMealRepositoryImpl` вместо `Map<Integer, Meal>`
>   - обратите внимание на `InMemoryBaseRepositoryImpl.counter` - счетчик один, общий для всех хранимых объектов
...
В данном случае двойная мапа (мультимап) - самый эффективный способ хранения, который не требует итерирования (перебора всех значений). С другой стороны затраты по памяти больше, поэтому решение не подходит, если юзеров много.
...
-  <a href="http://java-course.ru/begin/postgresql/">Установка PostgreSQL</a>. **ВНИМАНИЕ! с Postgres 11 возможны проблемы.** 
